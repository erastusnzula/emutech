...
class Item(models.Model):
    ...
    discount = models.IntegerField(default=5, blank=True, null=True)
...
class CartItem(models.Model):
    ...
    def get_price(self):
        if self.item.discount:
            price = float(self.item.price) * float(self.quantity)
            discount = float(self.item.discount) / 100 * float(price)
